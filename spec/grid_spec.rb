...
RSpec.describe Grid do
  ...
  context "Updates grid" do
    it "marks an 'X' on the 1st position of the grid" do
      expect(grid.place_a_move(unmarked_grid, 0, "X")).to eq(["X", 2, 3, 4, 5, 6, 7, 8, 9])
    end

    it "marks an 'X' on the 2nd position of the grid" do
      expect(grid.place_a_move(unmarked_grid, 1, "X")).to eq([1, "X", 3, 4, 5, 6, 7, 8, 9])
    end

    it "marks an 'X' on the 3rd position of the grid" do
      expect(grid.place_a_move(unmarked_grid, 2, "X")).to eq([1, 2, "X", 4, 5, 6, 7, 8, 9])
    end

    it "marks 'O' on the 1st position of the grid" do
      expect(grid.place_a_move(unmarked_grid, 0, "O")).to eq(["O", 2, 3, 4, 5, 6, 7, 8, 9])
    end
  end

  context "Checks the status of a move" do
    it "returns true if move is unique" do
      expect(grid.move_unique?(0, unmarked_grid)).to eq(true)
    end

    it "returns false if move is not unique" do
      current_grid = ["X", 2, 3, 4, 5, 6, 7, 8, 9]
      expect(grid.move_unique?(0, current_grid)).to eq(false)
    end

    it "returns true if move is unique and move is 2" do
      current_grid = ["X", 2, 3, 4, 5, 6, 7, 8, 9]
      expect(grid.move_unique?(1, current_grid)).to eq(true)
    end
  end
